...
SAVE_SINGLE_MODEL="${SAVE_SINGLE_MODEL:-0}"

SAVE_FLAG=()
if [[ "$SAVE_SINGLE_MODEL" == "1" ]]; then
  SAVE_FLAG=(--save-single-model)
elif [[ "$SAVE_SINGLE_MODEL" != "0" ]]; then
  echo "SAVE_SINGLE_MODEL must be 0 or 1, got: $SAVE_SINGLE_MODEL" >&2
  exit 2
fi

mapfile -t CONFIGS < <(python scripts/make_pacs_configs.py   --dataset domainnet   --seed "$SEED"   --target "$TARGET"   --method "$METHOD"   --dataset-path "$DATASET_PATH"   --output-root "$OUTPUT_ROOT"   "${SAVE_FLAG[@]}")
